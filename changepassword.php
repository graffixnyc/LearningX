...
if (!empty($_POST)){
	if(isset($_POST['username'])) {
		...
		if ($user[0]["userFound"]==1){
			...
			echo 'currentpw: ' . md5($curpass);
			echo 'newpass: ' . md5($_POST["password"]);
			echo 'email: ' . $useremail;
			echo 'reset: ' . $resetflag;
			$changepass=changePassword(md5($curpass),md5($_POST["password"]),$useremail,$resetflag);
		...
		<center>
					...
					if (isset($_SESSION["pwupdate"])){
						if ($_SESSION["pwupdate"]==0){
							...
							echo'<h2 style="color:salmon">Something went wrong. Wrong current password perhaps? </h2>';
							
						}
						else{
							echo'<h2 style="color:salmon">Password Changed.  Logging out in 2 seconds</h2>';
							echo '<meta http-equiv="refresh" content="2;URL=http://xfactor.abstractlogic.nyc/logout.php">';
						}
